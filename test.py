...
def main():
    filter_date = input("処理用の日付を YYYY-MM-DD 形式で入力してください: ")
    run_script_A(filter_date)
    run_script_B(filter_date)

# ---------------------------
# 共通: Notion API 設定
# ---------------------------

notion_token = "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"
notion_headers = {
...
def run_script_A(filter_date: str):
    ...
    created, updated, errors = process_records(
        filter_date=filter_date,
        k_sub="n2amf",
        k_app="52",
        k_token="MKIsBor99WYeG1uAEkV3ADZllkRKHRg6zAojyF8X",
        notion_db_id="1a74dbc3b61180ceb45ad2784be4d549",
        field_mapping=field_mapping
    )

    print(f"[Script A] 完了: 新規 {created} 件, 更新 {updated} 件, エラー {errors} 件")
...
def run_script_B(filter_date: str):
    ...
    created, updated, errors = process_records(
        ...
        k_sub="n2amf",
        k_app="31",
        k_token="DAXTQLrv5spCMHXNiVBWk4YewXrDEsnQTus8sQT0",
        notion_db_id="1ce4dbc3b61180c4899ecaa6feca4800",
        field_mapping=field_mapping
    )

    print(f"[Script B] 完了: 新規 {created} 件, 更新 {updated} 件, エラー {errors} 件")
